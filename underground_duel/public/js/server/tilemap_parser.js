...
export async function parseTilemapFile(fileSrc) {
    ...
    for (let l = 0; l < layers.length; l++) {
        ...
        for (let y = 0; y < tilemapJson.height; y++) {
            ...
            for (let x = 0; x < tilemapJson.width; x++) {
                ...
                row.push(new TileModel(new Vector2D(x * Settings.tile.tileSize + tempShiftX, y * Settings.tile.tileSize + tempShiftY), new Vector2D(Settings.tile.tileSize, Settings.tile.tileSize), new Vector2D(x, y), imageIndex, tilesetName));
            }
            layer.push(row);
        }
...
//# sourceMappingURL=tilemap_parser.js.map